...
RD Gateway's transport doesn't use ordinary HTTP verbs. It opens two long-lived requests with the custom methods `RDG_IN_DATA` and `RDG_OUT_DATA`, and Cloudflare's edge runs an HTTP method allowlist, so those two come back `501`, generated at the edge, before the request is ever forwarded. You can confirm it yourself against any proxied hostname:
...
It still gives you a public hostname, which is the part that gets missed. The VPS has a
public IPv4 of its own, so `rdg.yourdomain.tld` is an ordinary A record pointing at it,
...
nginx runs in `stream` mode with `ssl_preread`, which reads the SNI to pick a backend without decrypting anything. TLS terminates on the Windows box, exactly as it would with a port-forward. So the relay never holds your certificate and cannot read a byte of your session. Your win-acme setup on the gateway keeps working untouched, since it validates over Cloudflare DNS-01 and never needed inbound 80 or 443 anyway. And because nothing on the path parses HTTP, the custom-verb problem doesn't arise.
...
**2. Move the env file** to the Proxmox host over scp. It contains a private key, so don't paste it through a chat window, and shred it on both ends when you're done.
...
From anywhere off your LAN, and a phone on cellular is the honest test, since consumer routers often don't hairpin:
...
- [FreeRDP #6731](https://github.com/FreeRDP/FreeRDP/issues/6731), on what a reverse proxy has to get right for `RDG_IN_DATA`: no chunked re-encoding, no buffering either direction
- [ngx_stream_proxy_module](https://nginx.org/en/docs/stream/ngx_stream_proxy_module.html), for `proxy_timeout`, `proxy_responses` and UDP proxying
...
- [Cloudflare community: RD Gateway via proxied](https://community.cloudflare.com/t/microsoft-remote-desktop-gateway-via-proxied/245148), which works grey-clouded and fails orange
